...
EB_case3 = EB_step{CIndex,start};%The list of number of existed buildings in each station (1,2,...,7) in order.

% 2. ET: Existed Track Form (Direction Sensitive)
ET_case3 = ET_step{CIndex,start};%Case 1 existed track form, ET(i,j) = the number of tracks between station i and j.
  
% 3. EW: Existed Way Form (In the future, we could add some cal code for t)
EW_case3 = EW_step{CIndex,start};%Note EW is always symmetric, i.e. EW=EW'
  
% 4. EP_A: Existed Path Volume for Shipment A
%    EP_B: Existed Path Volume for Shipment B           
EP_case3 = EP_step{CIndex,start};
          
% 5. ESV_A: Existed Station Volume for Shipment A
%    ESV_B: Existed Station Volume for Shipment B
ESV_case3 = ESV_step{CIndex,start};

u=2;% Input the unit you want to ship.
start=1;% Input the starting point.
k=0;% Input the intermidiate points going through
e=13;% Input end point.
...
for k=0:length(ESLx_case1)-2 %We don't need to go to more steps than that, otherwise must have repeated points or self loops.
    ...
    if k == 0
        for start=1:length(ESLx_case1)
            if start ~= e
                [cost_new,EW_new,ET_new,EB_new,EP_new,ESV_new]=OptCost(u,start,k,e,Dst_case1,EW_case3,ET_case3,EB_case3,PL_case1,SL_case1,EP_case3,ESV_case3,Ct_case1,Cp_case1,Cr_case1,L_case1);
    ...
    % Step 2,3... and so on; calculate and store all the possible starting point s to destination e, going through 1 point.
        ...
        for start=1:length(ESLx_case1)
            ...
            if start ~= e
                ...
                [cost_new,EW_new,ET_new,EB_new,EP_new,ESV_new]=OptCost(u,start,0,i,Dst_case1,EW_step{k,i},ET_step{k,i},EB_step{k,i},PL_case1,SL_case1,EP_case3,ESV_step_temp{k,i},Ct_case1,Cp_case1,Cr_case1,L_case1);
...
disp('Third step, ship 2 units A from station 1 to station 13')

%Display the minimal cost from starting point to the endpoint
[MinCost3,CIndex]=min(cost_step(:,start));
MinCost3

%Display the ultimate Reoloadings, EB.
disp('Third step, the updated Reoloading Stations')
EB_step{CIndex,start}

%Display the Reoloading changes, delta~EB.
disp('Third step, the Reoloading Station changes')
EB_step{CIndex,start}-EB_case3

%Display the ultimate Way Pavements, EW.
disp('Third step, the updated Way Pavements')
EW_step{CIndex,start}

%Display the Way Pavement changes, delta~EW.
disp('Third step, the Way Pavement changes')
EW_step{CIndex,start}-EW_case3

%Display the updated Tracks, ET.
disp('Third step, the updated Tracks')
ET_step{CIndex,start}

%Display the updated Tracks, delta~ET.
disp('Third step, the Track changes')
ET_step{CIndex,start}-ET_case3

%Display The total cost and total plan
disp('The total minimal cost for PHASE II')
TotalMinCost=MinCost1+MinCost2+MinCost3
